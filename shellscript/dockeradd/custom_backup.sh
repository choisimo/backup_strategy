...
display_help() {
    ...
    echo "  --docker                   Docker 컨테이너 백업 활성화"
    echo "  --docker-volumes [VOLUMES] 백업할 Docker 볼륨 지정 (기본값: all)"
    echo "  --docker-compose-dir [DIR] Docker Compose 파일 위치 (기본값: /opt/docker)"
    echo "  --backup-images            Docker 이미지 백업 활성화"
    echo "  --stop-containers          백업 전 컨테이너 중지"
    echo "  --ignore-containers [LIST] 백업에서 제외할 컨테이너 목록"
    echo
    echo "예시:"
    ...
    echo "# 전체 Docker 환경 백업 (볼륨+이미지+compose)
    ./backup.sh --docker --backup-images --docker-compose-dir /my-compose --stop-containers

    # 선택적 볼륨 백업 (DB만)
    ./backup.sh --docker --docker-volumes db_data --ignore-containers redis

    # 원격 서버 + Docker 백업
    ./backup.sh --docker -e user@backup-server:/backups --stop-containers
    "
    echo
    exit 0
}
...
validate_input() {
    [ -d "$BACKUP_SOURCE_DIR" ] || { echo -e "${RED}소스 디렉토리 없음${NC}"; exit 1; }
    [[ "$RETENTION_DAYS" =~ ^[0-9]+$ ]] || { echo -e "${RED}유효하지 않은 보존 일수${NC}"; exit 1; }
}
...
read_ssh_config() {
    ...
    if [ ! -z "$IDENTITY_FILE" ]; then
        key_perm=$(stat -c %a "$IDENTITY_FILE")
        if [ "$key_perm" -ne 600 ]; then
            echo -e "${RED}위험: SSH 키 파일 권한이 600이 아닙니다 ($key_perm).${NC}"
            exit 1
        fi
    fi  
...
backup_docker_compose() {
    echo "Docker Compose 설정 파일 백업 중..." | tee -a "$LOG_FILE"
    
    for compose_file in $(find "$DOCKER_COMPOSE_DIR" -name "docker-compose*.yml" -o -name "docker-compose*.yaml"); do
        local rel_path=${compose_file#$DOCKER_COMPOSE_DIR}
        local target_dir=$(dirname "$BACKUP_DEST_DIR/compose$rel_path")
...
backup_docker_volumes() {
    ...
    for volume in "${volumes[@]}"; do
        echo "볼륨 백업 전 컨테이너 중지: $volume"
        docker stop $(docker ps -q --filter volume=$volume) 2>/dev/null || true
        
        # 실제 백업 로직
        docker run --rm -v $volume:/data -v $BACKUP_DEST_DIR:/backup \
          alpine tar czf /backup/$volume-$(date +%Y%m%d).tar.gz -C /data .
        
        echo "볼륨 백업 후 컨테이너 재시작"
        docker start $(docker ps -aq --filter volume=$volume) 2>/dev/null || true
    done
}
...
perform_backup() {
    MAX_LOG_FILES=30
    find "$BACKUP_LOG_DIR" -name "*.log" -mtime +30 -exec rm -f {} \;
...
stop_docker_containers() {
    if [ "$STOP_CONTAINERS" = true ]; then
        echo -e "${BLUE}Docker 컨테이너 중지 중...${NC}"
        docker stop $(docker ps -q --filter "name=${IGNORE_CONTAINERS//,/ --filter name!=}") 2>/dev/null || true
    fi
}

start_docker_containers() {
    if [ "$STOP_CONTAINERS" = true ]; then
        echo -e "${BLUE}Docker 컨테이너 재시작 중...${NC}"
        docker start $(docker ps -aq --filter "name=${IGNORE_CONTAINERS//,/ --filter name!=}") 2>/dev/null || true
    fi
}

backup_docker_images() {
    echo -e "${BLUE}Docker 이미지 백업 시작${NC}"
    local image_dir="$BACKUP_DEST_DIR/docker-images"
    mkdir -p "$image_dir"
...
advanced_docker_volume_backup() {
    local volume=$1
    echo -e "${BLUE}고급 볼륨 백업: $volume${NC}"
    
    docker run --rm -v "$volume:/volume_data" -v "$BACKUP_DEST_DIR:/backup" \
      alpine sh -c "apk add sqlite && \
      tar czf /backup/$volume-$(date +%Y%m%d).tar.gz -C /volume_data . --checkpoint=1000"
}
